...
func NewFromConfig(l logger.Logger, cfg config.Sink, acker cdc.Acker) (Sink, error) {
	switch cfg.Type {
	case config.SinkTypeLocalFile:
		...
		return localfilesink.NewLocalFileSink(l, cfg.LocalFile.DestinationDir, acker), nil
	default:
