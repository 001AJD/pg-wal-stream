...
func TestSinkDispatcherDispatchesToSinkHandler(t *testing.T) {
	...
	dispatcher := NewSinkDispatcher(sink.NewHandler(jsonl.NewEncoder(), nil, target))
...
func TestSinkDispatcherReturnsSinkError(t *testing.T) {
	...
	dispatcher := NewSinkDispatcher(sink.NewHandler(jsonl.NewEncoder(), nil, &failingSink{err: sinkErr}))
